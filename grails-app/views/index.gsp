<!DOCTYPE html>
<html lang="en">
    <head>
        <meta name="layout" content="main"/>
    </head>
    <body>
    <div class="container" style="width: 100%;">
        <h1 class="indexTitle">Asegurese su tranquilidad.<br/>40 años de experiencia en el servicio</h1>
    </div>
    <g:img dir="images" file="background.png" width="100%" height="100px" style="margin-top: 60px"/>
    <div class="row" style="margin-top: -130px; padding-left: 15%; width: 100%">
        <div class="col-lg-2">
            <div class="image-circle" style="background-color: #208554; ">
                <h2  class="circle-font" style=" padding-top: 45px; ">¿QUIENES SOMOS?</h2>
            </div>
        </div>
        <div class="col-lg-2">
            <div class="image-circle" style="background-color: #cf233f;">
                <h2  class="circle-font" style=" padding-top: 45px;">PERSONAS NATURALES</h2>
            </div>
        </div>
        <div class="col-lg-2">
            <div class="image-circle" style="background-color: #13baa9;">
                <h2  class="circle-font" style=" padding-top: 45px;">PERSONAS JURIDICAS</h2>
            </div>
        </div>
        <div class="col-lg-2">
            <div class="image-circle" style="background-color: #4d6b70;">
                <h2  class="circle-font" style=" padding-top: 45px;">CUPOS DE AFINIDAD</h2>
            </div>
        </div>
        <div class="col-lg-2">
            <div class="image-circle" style="background-color: #e4b42c; ">
                <h2 class="circle-font" style=" padding-top: 55px;">ALIADOS</h2>
            </div>
        </div>
    </div>
    </body>
</html>
